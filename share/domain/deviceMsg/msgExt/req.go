...
type (
	...
	Payload struct {
		...
		RetEnc    string `json:"retEnc"`    //返回使用的加密方式: aes128cbc(默认) aes128ecb hex
	}

	RespData struct {
		Len int64 `json:"len"` //payload加密前信息的长度
		/*
		  默认加密过程将原始 JSON 格式的 payload 转为字符串后进行 AES 加密，再进行 base64 加密。AES 加密算法为 CBC 模式，密钥长度128，取 productSecret 前16位，偏移量为长度16的字符“0”。
		  retEnc 为 hex 时 payload 为明文 JSON 字符串，psk 为设备密钥原始字节的十六进制编码。
		  原始 payload 内容说明：
		  key                value               描述
