...
(
    ...
    `autoRegister` tinyint (
    1
) DEFAULT '1' COMMENT '动态注册:1:关闭,2:打开,3:打开并自动创建设备',
    `secret` varchar (
    50
) DEFAULT '' COMMENT '动态注册产品秘钥',
    `desc` varchar (
    200
) DEFAULT '' COMMENT '描述',
...
(
    ...
    UNIQUE KEY `productID_identifier` (
    `productID`,
    `identifier`
),
    KEY `productID_type` (
...
(
    ...
    `logLevel` tinyint (
    1
) DEFAULT '1' COMMENT '日志级别:1)关闭 2)错误 3)告警 4)信息 5)调试',
    `cert` varchar (
    512
) DEFAULT '' COMMENT '设备证书',
    `isOnline` tinyint (
    1
) default 2 comment '是否在线,1是2否',
    `tags` json not null comment '设备标签',
    `address` varchar (
    512
) DEFAULT '' COMMENT '所在地址',
    `position` point NOT NULL comment '设备的位置,默认百度坐标系BD09',
    PRIMARY KEY (
    `id`
),
    UNIQUE KEY `productID_deviceName` (
    `productID`,
    `deviceName`
),
    KEY `createdTime` (
...
(
    ...
    `dir` varchar (
    128
) NOT NULL COMMENT '固件标识,拿来下载文件',
    PRIMARY KEY (
    `id`
),
    UNIQUE KEY `deviceVersion` (
    `productID`,
    `version`
)
    ) ENGINE = InnoDB
