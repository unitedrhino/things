...
(
    ...
    `productType`  int(10)   DEFAULT '0' COMMENT '产品状态:0:开发中,1:审核中,2:已发布',
    `authMode`     int(10)   DEFAULT '0' COMMENT '认证方式:0:账密认证,1:秘钥认证',
    `deviceType`   int(10)   DEFAULT '0' COMMENT '设备类型:0:设备,1:网关,2:子设备',
    `categoryID`   int(10)   DEFAULT '0' COMMENT '产品品类',
    `netType`      int(10)   DEFAULT '0' COMMENT '通讯方式:0:其他,1:wi-fi,2:2G/3G/4G,3:5G,4:BLE,5:LoRaWAN',
    `dataProto`    int(10)   DEFAULT '0' COMMENT '数据协议:0:自定义,1:数据模板',
    `autoRegister` int(10)   DEFAULT '0' COMMENT '动态注册:0:关闭,1:打开,2:打开并自动创建设备',
