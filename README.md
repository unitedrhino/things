...
基于现有图纸、CAD、设备台账和计量关系，可构建建筑与配电站可交互三维场景，并在完成设备绑定和计量配置后接入真实数据。演示画面中的读数为录制时数据，不代表当前实时数据。

**建筑能耗：整楼、楼层与计量范围联查**

![建筑能耗数字孪生演示](./doc/assets/cases/智慧能源-建筑数字孪生.svg)

**配电站：柜列、仪表读数与历史趋势联查**

![配电站数字孪生演示](./doc/assets/cases/智慧能源-配电站数字孪生.svg)

#### 五类能源大屏模板示例

平台内置电力、水务、燃气、热力、燃煤五类能源大屏系统模板，从模板一键创建即可获得同款大屏。设备数据经 MQTT 接入、WebSocket 实时推送，告警由规则引擎按阈值触发并支持处置详情。

![能源大屏-电力](./doc/assets/bigscreen/能源大屏-电力.png)

其余四类同构布局、按品类差异化配色与数据口径：
