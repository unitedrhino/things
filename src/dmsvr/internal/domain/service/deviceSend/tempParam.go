...
func (t *TempParam) AddDefine(d *deviceTemplate.Define, val interface{}) (err error) {
	t.Value.Type = d.Type
	t.Value.Type = d.Type
...
func GetVal(d *deviceTemplate.Define, val interface{}) (interface{}, error) {
	switch d.Type {
	...
	case deviceTemplate.ENUM: //枚举类型 报文中传递的是数字
		if num, ok := val.(json.Number); !ok {
			return nil, errors.Parameter.AddDetail(val)
		} else {
			ret, err := num.Int64()
