...
type (
	...
	DeviceDataRepo interface {
		...
		InsertPropertyData(ctx context.Context, t *deviceTemplate.Template, productID string, deviceName string, property *PropertyData) error
		// InsertPropertiesData 插入多条属性数据 params key为属性的id,val为属性的值
		InsertPropertiesData(ctx context.Context, t *deviceTemplate.Template, productID string, deviceName string, params map[string]interface{}, timestamp time.Time) error
		...
		InitProduct(ctx context.Context, t *deviceTemplate.Template, productID string) error
		// DropProduct 删除产品时需要删除产品下的所有表
		DropProduct(ctx context.Context, t *deviceTemplate.Template, productID string) error
		// InitDevice 创建设备时为设备创建单独的表
		InitDevice(ctx context.Context, t *deviceTemplate.Template, productID string, deviceName string) error
		// DropDevice 删除设备时需要删除设备的所有表
		DropDevice(ctx context.Context, t *deviceTemplate.Template, productID string, deviceName string) error
		// ModifyProduct 修改产品物模型 只支持新增和删除,不支持修改数据类型
		ModifyProduct(ctx context.Context, oldT *deviceTemplate.Template, newt *deviceTemplate.Template, productID string) error
	}
)
