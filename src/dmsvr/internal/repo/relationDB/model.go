...
type DmProductSchema struct {
	...
	Type       int64  `gorm:"column:type;type:smallint;default:1"`          // 物模型类型 1:property属性 2:event事件 3:action行为
...
type DmGroupInfo struct {
	GroupID   int64             `gorm:"column:group_id;type:bigint"`                               // 分组ID
	...
	GroupName string            `gorm:"column:group_name;type:varchar(100);NOT NULL"`              // 分组名称
	Desc      string            `gorm:"column:desc;type:varchar(200)"`                             // 描述
...
type DmGroupDevice struct {
	ID         int64  `gorm:"column:id;type:bigint;primary_key;AUTO_INCREMENT"`
	GroupID    int64  `gorm:"column:group_id;type:bigint;NOT NULL"`             // 分组ID
	ProjectID  int64  `gorm:"column:project_id;type:bigint;default:0;NOT NULL"` // 项目ID(雪花ID)
	ProductID  string `gorm:"column:product_id;type:char(11);NOT NULL"`         // 产品id
	DeviceName string `gorm:"column:device_name;type:varchar(100);NOT NULL"`    // 设备名称
	stores.Time
