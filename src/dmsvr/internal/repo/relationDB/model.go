...
type DmProductInfo struct {
	...
	ProductType  int64             `gorm:"column:product_type;type:tinyint(1);default:1"`             // 产品状态:1:开发中,2:审核中,3:已发布
	AuthMode     int64             `gorm:"column:auth_mode;type:tinyint(1);default:1"`                // 认证方式:1:账密认证,2:秘钥认证
	DeviceType   int64             `gorm:"column:device_type;type:tinyint(1);default:1"`              // 设备类型:1:设备,2:网关,3:子设备
	CategoryID   int64             `gorm:"column:category_id;type:int(10);default:1"`                 // 产品品类
	NetType      int64             `gorm:"column:net_type;type:tinyint(1);default:1"`                 // 通讯方式:1:其他,2:wi-fi,3:2G/3G/4G,4:5G,5:BLE,6:LoRaWAN
	DataProto    int64             `gorm:"column:data_proto;type:tinyint(1);default:1"`               // 数据协议:1:自定义,2:数据模板
	AutoRegister int64             `gorm:"column:auto_register;type:tinyint(1);default:1"`            // 动态注册:1:关闭,2:打开,3:打开并自动创建设备
...
type DmProductCustom struct {
	...
	ProductID       string         `gorm:"column:product_id;type:char(11);NOT NULL"`     // 产品id
	ScriptLang      int64          `gorm:"column:script_lang;type:tinyint(1);default:1"` // 脚本语言类型 1:JavaScript 2:lua 3:python
...
type DmDeviceInfo struct {
	ID             int64             `gorm:"column:id;type:bigint(20);primary_key;AUTO_INCREMENT"`
	ProjectID      int64             `gorm:"column:project_id;type:bigint(20);default:0;NOT NULL"`      // 项目ID(雪花ID)
	AreaID         int64             `gorm:"column:area_iD;type:bigint(20);default:0;NOT NULL"`         // 项目区域ID(雪花ID)
	ProductID      string            `gorm:"column:product_id;type:char(11);NOT NULL"`                  // 产品id
	...
	MobileOperator int64             `gorm:"column:mobile_operator;type:tinyint(1);default:1;NOT NULL"` // 移动运营商:1)移动 2)联通 3)电信 4)广电
	...
	IsOnline       int64             `gorm:"column:is_online;type:tinyint(1);default:2;NOT NULL"`       // 是否在线,1是2否
	FirstLogin     sql.NullTime      `gorm:"column:first_login;type:datetime"`                          // 激活时间
	LastLogin      sql.NullTime      `gorm:"column:last_login;type:datetime"`                           // 最后上线时间
	LogLevel       int64             `gorm:"column:log_level;type:tinyint(1);default:1;NOT NULL"`       // 日志级别:1)关闭 2)错误 3)告警 4)信息 5)调试
	stores.Time
	ProductInfo *DmProductInfo `gorm:"foreignKey:product_id;references:product_id"`
}
...
type DmGatewayDevice struct {
	ID                int64  `gorm:"column:id;type:bigint(20);primary_key;AUTO_INCREMENT"`
	GatewayProductID  string `gorm:"column:gateway_product_id;type:char(11);NOT NULL"`      // 网关产品id
