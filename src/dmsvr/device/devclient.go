...
func (d *DevClient) DeviceResp(Method, ClientToken string, topics []string, err error, data map[string]interface{}) {
	...
	payload, _ := json.Marshal(DeviceResp{
		Method:      respMethod,
...
func (d *DevClient) DeviceReq(ctx context.Context, req DeviceReq, pubTopic, subTopic string) (*DeviceResp, error) {
	...
	for {
		...
		case resp := <-respInfo.Msg:
			return resp, nil
		case <-ctx.Done():
			return &DeviceResp{}, errors.DeviceTimeOut
		}
	}
}

//服务器已经向设备发送了请求,这个是把回复送给对应的处理函数
func (d *DevClient) DeviceReqSendResp(resp *DeviceResp, topic string) error {
