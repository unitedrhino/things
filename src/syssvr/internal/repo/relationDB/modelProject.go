...
type SysUserArea struct {
	...
	UserID     int64             `gorm:"column:userID;uniqueIndex:ri_mi;type:bigint;NOT NULL"`           // 用户ID(雪花id)
	ProjectID  int64             `gorm:"column:projectID;uniqueIndex:ri_mi;type:bigint;NOT NULL"`        // 所属项目ID(雪花ID)
	AreaID     int64             `gorm:"column:areaID;uniqueIndex:ri_mi;type:bigint;NOT NULL"`           // 区域ID(雪花ID)
	RoleType   int64             `gorm:"column:roleType;type:bigint;NOT NULL"`                           // 角色类型 1 管理员(可以调整本区域及旗下区域的设备区域规划)  2 读写授权(可以对区域下的设备进行操作,但是不能新增或删除)
	stores.NoDelTime
	DeletedTime stores.DeletedTime `gorm:"column:deleted_time;index"`
}
