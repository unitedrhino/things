...
func (l *UserRegister2Logic) UserRegister2(in *sys.UserRegister2Req) (*sys.Response, error) {
	...
	case users.RegWxOpen:
		_, err = l.UiDB.FindOneByFilter(l.ctx, relationDB.UserInfoFilter{WechatUnionID: tokenInfo.Note})
		if err == nil { //已经注册过
		...
		userInfo.WechatUnionID = sql.NullString{String: tokenInfo.Note, Valid: true}
	}
