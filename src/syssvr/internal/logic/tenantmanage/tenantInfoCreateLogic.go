...
// 新增区域
func (l *TenantInfoCreateLogic) TenantInfoCreate(in *sys.TenantInfo) (*sys.Response, error) {
	err := logic.IsSupperAdmin(l.ctx, def.TenantCodeDefault)
	if err != nil {
		return nil, err
	}
	err = relationDB.NewTenantInfoRepo(l.ctx).Insert(l.ctx, ToTenantInfoPo(in))
