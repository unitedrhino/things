...
func (s *Server) ListenTCPServer(addr string) {
	...
	if err != nil {
		s.Logger.Errorf("net.ResolveTCPAddr err", err, addr)
	}
	s.port = NewPort(tcpaddr.Port)
	s.host, err = utils.ResolveSelfIP()
	if err != nil {
		s.Logger.Errorf("net.ListenTCP resolveip err", err, addr)
	}
	tcplistener, err := net.ListenTCP("tcp", tcpaddr)
	if err != nil {
		s.Logger.Errorf("net.ListenTCP err", err, addr)
	}
	defer tcplistener.Close()
	//conn, err := tcplistener.Accept()
	//s.conn = newTCPConnection(conn)
	var (
		raddr net.Addr
		num   int
	)
	...
	for {
		conn, err := tcplistener.Accept()
		s.conn = newTCPConnection(conn) //tcp短链接
		if err != nil {
		...
		if err != nil {
			s.Logger.Errorf("udp.ReadFromTCP err", err)
			continue
		}
