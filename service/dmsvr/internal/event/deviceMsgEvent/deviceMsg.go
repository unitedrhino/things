...
func (l *DeviceMsgHandle) deviceResp(di *dm.DeviceInfo, respMsg *deviceMsg.PublishMsg) {
	...
	l.svcCtx.ScriptTrans.DownAfterTrans(l.ctx, di, respMsg)
...
func (l *DeviceMsgHandle) FixDisconnect(msg *deviceMsg.PublishMsg) {
	...
	ctxs.GoNewCtx(l.ctx, func(ctx context.Context) {
		...
		if di.IsOnline != def.True { //如果不在线但是上报了需要调整为在线状态
			err = relationDB.NewDeviceInfoRepo(ctx).UpdateConnectivityOnlineByID(
				ctx, di.Id, time.UnixMilli(msg.Timestamp), di.LastIp, di.FirstLogin == 0)
			if err != nil {
				l.Error(err)
				return
			}
