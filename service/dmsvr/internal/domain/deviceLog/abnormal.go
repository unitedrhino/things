...
type (
	Abnormal struct {
		TenantCode  string                 `gorm:"-"` // 租户编码
		ProjectID   int64                  `gorm:"-"` // 项目ID(雪花ID)
		AreaID      int64                  `gorm:"-"` // 项目区域ID(雪花ID)
		AreaIDPath  string                 `gorm:"-"` // 项目区域ID路径(雪花ID)
		BelongGroup map[string]def.IDsInfo `gorm:"-"`
