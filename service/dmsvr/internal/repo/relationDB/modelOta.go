...
type DmOtaFirmwareDevice struct {
	...
	JobID           int64               `gorm:"column:job_id;index:idx_dm_ota_firmware_device_job_deleted_device;uniqueIndex:tc_un;type:BIGINT" `            // 升级批次ID
	ProductID       string              `gorm:"column:product_id;type:varchar(20);uniqueIndex:tc_un"`                                                        // 设备所属产品的productID
	DeviceName      string              `gorm:"column:device_name;index:idx_dm_ota_firmware_device_job_deleted_device;type:varchar(100);uniqueIndex:tc_un" ` // 设备名称
	...
	DeletedTime stores.DeletedTime   `gorm:"column:deleted_time;default:0;index:idx_dm_ota_firmware_device_job_deleted_device;uniqueIndex:tc_un;"`
