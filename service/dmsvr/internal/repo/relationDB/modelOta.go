...
type DmOtaFirmwareInfo struct {
	...
	stores.NoDelTime
	DeletedTime stores.DeletedTime `gorm:"column:deleted_time;default:0;uniqueIndex:idx_dm_ota_firmware_info_tc_un;"`
}
...
type DmOtaFirmwareDevice struct {
	TenantCode      dataType.TenantCodeWithDefaultR `gorm:"column:tenant_code;index;type:VARCHAR(50);default:'default'"` // 租户编码
	ID              int64                          `gorm:"column:id;type:BIGINT;primary_key;AUTO_INCREMENT"`
	FirmwareID      int64                          `gorm:"column:firmware_id;uniqueIndex:idx_dm_ota_firmware_device_tc_un;index:idx_dm_ota_fw_device_push_lookup,priority:1"`                       // 升级包ID
	SrcVersion      string                         `gorm:"column:src_version;type:varchar(125)"`                                                                                                   // 设备的原固件版本
	DestVersion     string                         `gorm:"column:dest_version;type:varchar(125);index:idx_dm_ota_fw_device_report_lookup,priority:4"`                                             // 设备的目标固件版本
	JobID           int64                          `gorm:"column:job_id;index:idx_dm_ota_firmware_device_job_deleted_device;uniqueIndex:idx_dm_ota_firmware_device_tc_un;index:idx_dm_ota_fw_device_push_lookup,priority:2;type:BIGINT" `            // 升级批次ID
	ProductID       string                         `gorm:"column:product_id;type:varchar(20);uniqueIndex:idx_dm_ota_firmware_device_tc_un;index:idx_dm_ota_fw_device_report_lookup,priority:1;index:idx_dm_ota_fw_device_push_lookup,priority:3"`    // 设备所属产品的productID
	DeviceName      string                         `gorm:"column:device_name;index:idx_dm_ota_firmware_device_job_deleted_device;type:varchar(100);uniqueIndex:idx_dm_ota_firmware_device_tc_un;index:idx_dm_ota_fw_device_report_lookup,priority:2;index:idx_dm_ota_fw_device_push_lookup,priority:6" ` // 设备名称
	...
	Status          msgOta.DeviceStatus            `gorm:"column:status;index;type:BIGINT;index:idx_dm_ota_fw_device_report_lookup,priority:3;index:idx_dm_ota_fw_device_push_lookup,priority:4"`  // 设备升级作业状态。1：待确认。 2：待推送。 3：已推送。  4：升级中。 5:升级成功 6: 升级失败. 7:已取消
	...
	DeletedTime stores.DeletedTime   `gorm:"column:deleted_time;default:0;index:idx_dm_ota_firmware_device_job_deleted_device;uniqueIndex:idx_dm_ota_firmware_device_tc_un;index:idx_dm_ota_fw_device_report_lookup,priority:5;index:idx_dm_ota_fw_device_push_lookup,priority:5"`
...
type DmOtaModuleInfo struct {
	ID        int64  `gorm:"column:id;type:bigint;primary_key;AUTO_INCREMENT"`
	ProductID string `gorm:"column:product_id;uniqueIndex:idx_dm_ota_module_info_tc_un;type:varchar(20)" ` // 设备所属产品的productID
	Code      string `gorm:"column:code;uniqueIndex:idx_dm_ota_module_info_tc_un;type:varchar(64)"`
	Name      string `gorm:"column:name;type:varchar(64)"`  // 固件名称
	Desc      string `gorm:"column:desc;type:varchar(200)"` // 描述
	stores.NoDelTime
	DeletedTime stores.DeletedTime `gorm:"column:deleted_time;default:0;uniqueIndex:idx_dm_ota_module_info_tc_un;"`
}
