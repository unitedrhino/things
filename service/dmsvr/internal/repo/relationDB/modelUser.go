...
type DmUserDeviceCollect struct {
	...
	UserID     int64               `gorm:"column:user_id;type:BIGINT;uniqueIndex:idx_dm_user_device_collect_product_id_deviceName;NOT NULL"`           // 问题提出的用户
	ProductID  string              `gorm:"column:product_id;type:varchar(100);uniqueIndex:idx_dm_user_device_collect_product_id_deviceName;NOT NULL"`  // 产品id
	DeviceName string              `gorm:"column:device_name;uniqueIndex:idx_dm_user_device_collect_product_id_deviceName;type:varchar(100);NOT NULL"` // 设备名称
	stores.NoDelTime
	DeletedTime stores.DeletedTime `gorm:"column:deleted_time;default:0;uniqueIndex:idx_dm_user_device_collect_product_id_deviceName"`
}
...
type DmUserDeviceShare struct {
	ID                int64               `gorm:"column:id;type:bigint;primary_key;AUTO_INCREMENT"`
	TenantCode        dataType.TenantCode `gorm:"column:tenant_code;index;type:VARCHAR(50);NOT NULL"`                                                    // 租户编码
	SharedUserID      int64               `gorm:"column:shared_user_id;type:BIGINT;uniqueIndex:idx_dm_user_device_share_product_id_deviceName;NOT NULL"` // 分享对象的用户ID
	...
	stores.NoDelTime
	DeletedTime stores.DeletedTime `gorm:"column:deleted_time;default:0;uniqueIndex:idx_dm_user_device_share_product_id_deviceName"`
}
