...
type DmDeviceInfo struct {
	...
	RatedPower     int64             `gorm:"column:rated_power;type:bigint;NOT NULL;default:0"`                               //额定功率:单位k/wh
...
type DmSchemaCore struct {
	...
	UserAuth          int64      `gorm:"column:user_auth;type:varchar(20);default:'rw'"`                      //用户权限操作: 1:r(只读) 3:rw(可读可写)
