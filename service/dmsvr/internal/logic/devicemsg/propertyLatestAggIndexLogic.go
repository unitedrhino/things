...
func (l *PropertyLatestAggIndexLogic) PropertyLatestAggIndex(in *dm.PropertyLatestAggIndexReq) (*dm.PropertyLatestAggIndexResp, error) {
	var (
	...
	dds, err := dd.GetPropertyLatestAgg(l.ctx, t, msgThing.FilterLatestAggOpt{
	...
	var diDatas []*dm.PropertyLatestAggResp
	for _, devData := range dds {
		diData := dm.PropertyLatestAggResp{
		...
		for _, v1 := range devData.Values {
			var dv = dm.PropertyLatestAggRespDetail{
				DataID:   v1.Identifier,
				DataName: schema.GetDataName(t, v1.Identifier),
				Values:   map[string]string{}}
			for k2, v2 := range v1.Values {
				var payload string
				if param, ok := v2.(string); ok {
					payload = param
				} else {
					payload = utils.ToString(v2)
				}
				dv.Values[k2] = payload
			}
			diData.Values = append(diData.Values, &dv)
		}
		diDatas = append(diDatas, &diData)
	}
	return &dm.PropertyLatestAggIndexResp{List: diDatas}, nil
}
