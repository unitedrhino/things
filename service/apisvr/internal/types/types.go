...
type DeviceMsgPropertyLogIndexReq struct {
	...
	TimeEnd     int64     `json:"timeEnd,string,optional,omitempty"`   //时间的结束
	Page        *PageInfo `json:"page,optional"`                       //分页信息
	Interval    int64     `json:"interval,optional"`                   //间隔(单位毫秒) 如果这个值不为零值 则时间的开始和结束必须有效及聚合函数不应该为空
	...
	Fill        string    `json:"fill,optional"`                       //指定窗口区间数据缺失的情况下的填充模式 参考:https://docs.taosdata.com/taos-sql/distinguished/
...
type ProductInfo struct {
	...
	IsCanCoverBindDevice int64                       `json:"isCanCoverBindDevice,optional"`         //是否可以覆盖绑定设备,默认为2
	ProtocolConf         []*Tag                      `json:"protocolConf,optional,omitempty"`       //协议配置
