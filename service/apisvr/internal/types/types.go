...
type DeviceInfoIndexReq struct {
	...
	PropertyIgnoreEmpty bool                `json:"propertyIgnoreEmpty,optional"`  // 设备属性是否忽略空值,空值是否返回
...
type DeviceInteractSendActionReq struct {
	...
	OutputParams string      `json:"outputParams,optional"` //输出参数(模拟的时候才需要填写) ,如果填写该参数,那么不会下发给设备,直接记录操作记录
	IsAsync      bool        `json:"isAsync,optional"`      //是否异步操作 异步情况通过获取接口来获取
...
type DeviceMsgPropertyAgg struct {
...
type GroupInfo struct {
	...
	Devices     []*DeviceCore     `json:"devices,optional,omitempty"` // withDevices=true 时返回分组下设备核心信息
...
type GroupInfoIndexReq struct {
	...
	Purpose     string    `json:"purpose,optional"`       //用途 不填默认为default
	WithDevices bool      `json:"withDevices,optional"`   // true 时返回 list[].devices 设备核心信息
}
...
type GroupInfoReadReq struct {
	...
	WithDevices  bool   `json:"withDevices,optional"` // true 时返回 devices 设备核心信息
}
