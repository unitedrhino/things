...
func (l *MultiCreateLogic) MultiCreate(req *types.UserDeviceShareMultiInfo) (resp *types.UserDeviceShareMultiToken, err error) {
	//将需要分享的设备记录缓存
	ret, err := l.svcCtx.UserDevice.UserDeviceShareMultiCreate(l.ctx, ToMuitlSharePb(req))
	if err != nil {
		return nil, err
	}
