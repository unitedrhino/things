...
var (
	...
	MediaSipUpdateError    = NewCodeError(MEDIA_ERROR+8, "ID或channelID不能都为空")
	MediaSipDevCreateError = NewCodeError(MEDIA_ERROR+9, "设备创建失败")
	MediaSipChnCreateError = NewCodeError(MEDIA_ERROR+10, "通道创建失败")
	MediaSipPlayError      = NewCodeError(MEDIA_ERROR+11, "通道播放失败")
)
